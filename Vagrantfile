...
Vagrant.configure("2") do |config|
  config.vm.define "whonix" do |whonix|
    ...
    whonix.vm.provision :shell, inline: <<-SCRIPT
      apt-get update && apt-get install -y dnsmasq netcat
      echo "interface=eth1\nport=0\ndhcp-range=10.152.152.11,10.152.152.11,infinite" > /etc/dnsmasq.conf
  ...
  config.vm.define "kali" do |kali|
    ...
    kali.vm.network :private_network, ip: "10.152.152.11", virtualbox__intnet: "Whonix", adapter: 1
    ...
    kali.vm.provision :shell, inline: <<-SCRIPT
      sudo nmcli connection modify "Wired connection 1" \
        ipv4.method manual \
        ipv4.addresses 10.152.152.11/18 \
